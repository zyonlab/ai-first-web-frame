...
/**
 * The portfolio layout owns theme/locale + renders the shared `AppNav` inside
 * its own React tree (fixing the shell-injected-chrome hydration mismatch,
 * React #418). Driven as an SSR server component with a mocked `next/headers`.
 */
...
async function renderLayout(): Promise<string> {
  ...
  const element = await RootLayout({
    children: createElement("main", { "data-page": "portfolio" }),
  });
  return renderToStaticMarkup(element);
}

describe("page-portfolio layout", () => {
  beforeEach(() => {
    cookieHeader = "";
  });
  ...
  it("marks the Portfolio link active via aria-current", async () => {
    const html = await renderLayout();
    expect(html).toMatch(/href="\/portfolio"[^>]*aria-current="page"/);
  });
