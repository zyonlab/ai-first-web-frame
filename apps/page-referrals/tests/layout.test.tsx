...
/**
 * The referrals layout owns theme/locale + renders the shared `AppNav` inside
...
async function renderLayout(): Promise<string> {
  ...
  const element = await RootLayout({
    children: createElement("main", { "data-page": "referrals" }),
  });
  return renderToStaticMarkup(element);
}

describe("page-referrals layout", () => {
  beforeEach(() => {
    cookieHeader = "";
  });
  ...
  it("marks the Referrals link active via aria-current", async () => {
    const html = await renderLayout();
    expect(html).toMatch(/href="\/referrals"[^>]*aria-current="page"/);
  });
