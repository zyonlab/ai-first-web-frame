...
| `ApiEndpointPolicySchema` / `RequestPolicySchema` | Allowed endpoints/methods/timeouts/retries for `@mvp/request` | Type-level only for the policy itself. Response *payloads* are opt-in validated per call: `requestJson`'s `responseSchema` option (any Zod-like `safeParse` carrier) parses the JSON body in `packages/request/src/index.ts` and throws a `RequestContractError` (url/endpoint/schema name/issues, never retried) on drift. Deliberately per-call and not part of the policy — policies are serializable contracts, runtime schemas are code. |
| `DataSource.responseSchema` (opt-in, structural — any Zod schema) | The payload a data source's `load` resolves | `packages/data/src/index.ts` — the load path (`readData`/`preloadData` and the `subscribeData` poll loop) `safeParse`s every loaded payload before caching/returning; a mismatch throws `DataDependencyError` naming the schema. Reference adoption: `account` (`AccountMarginSchema`, `domains/trade-data/src/tradeSources.ts`). |
