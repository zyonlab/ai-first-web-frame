...
| `RenderStrategySchema` + `normalizeRenderStrategy()` | Slot strategy enum `static\|isr\|ttl-cache\|cached-ssr\|dynamic-ssr` (`isr` = deprecated alias of `ttl-cache`) | Via the slot-element parsing above; normalized before comparison and inside cache keys in `packages/runtime/src/index.ts` (`fetchFragmentSlot`, `createFragmentCacheKey`). |
