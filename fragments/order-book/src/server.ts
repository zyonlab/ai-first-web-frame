...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderOrderBook({
      ctx: { locale: "en-US" },
      props: { symbol: "BTC", grouping: 1 },
    });
    reply.type("text/html; charset=utf-8");
    return renderServiceHome({
      title: "order-book fragment",
      port: DEFAULT_PORT,
  ...
  server.get("/manifest", async () => orderBookManifest);
  server.get("/assets", async () => orderBookManifest.assets);
  server.get("/budget", async () => orderBookBudget);
  server.post("/render", async (request, reply) => {
    ...
    const body = parsed.request as Parameters<typeof renderOrderBook>[0];
    ...
    const result = await renderOrderBook(body, { trace });
...
if (
  ...
  console.log(`order-book listening on ${port}`);
}
