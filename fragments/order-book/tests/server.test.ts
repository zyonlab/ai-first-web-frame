...
import { createOrderBookFallback } from "../src/render";
...
describe("order-book fragment service", () => {
  ...
  it("/assets declares the shared trade-client chunk + patch client + css", async () => {
    ...
    expect(assets.js).toContain("/assets/order-book.client.js");
  ...
  it("stamps metadata.fallback on the degraded render path", () => {
    const fallback = createOrderBookFallback("test-reason");
