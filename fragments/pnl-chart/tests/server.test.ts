...
// (`curve.test.ts`, `render.test.ts`) cover the fragment behavior without a
...
describe("pnl-chart fragment service", () => {
  it("/ returns a browser-readable service demo", async () => {
    const server = buildServer();
    const response = await server.inject({ method: "GET", url: "/" });
    expect(response.statusCode).toBe(200);
    expect(response.headers["content-type"]).toContain("text/html");
    expect(response.body).toContain("pnl-chart fragment");
    expect(response.body).toContain("POST http://localhost:4214/render");
  });
  ...
  it("/health returns ok with service and uptime", async () => {
    ...
    expect(body).toMatchObject({ status: "ok", service: "pnl-chart" });
  ...
  it("/manifest passes schema validation", async () => {
    ...
    expect(validatePnlChartManifest(response.json())).toBe(true);
  });
  ...
  it("/assets returns css and declares zero framework JS", async () => {
    ...
    const body = response.json();
    expect(body.css).toEqual(expect.arrayContaining(["/assets/pnl-chart.css"]));
    // Pure SSR inline SVG: no framework JS shipped.
    expect(body.js).toEqual([]);
  });

  it("/budget stays under the fragment JS/CSS hard gates", async () => {
    const server = buildServer();
  ...
  it("/render returns pnl SVG read via the data plane", async () => {
    const server = buildServer();
  ...
  it("/render exports a trace with a render span through the pipeline", async () => {
    ...
    await server.inject({
      ...
      url: "/render",
      payload: {
        ctx: { traceId: "trace-pnl-test" },
        props: { symbol: "BTC" },
      },
    });
    await new Promise((resolve) => setImmediate(resolve));
    expect(exported).toHaveLength(1);
    expect(exported[0].traceId).toBe("trace-pnl-test");
    const names = exported[0].nodes.map((node) => node.name);
    expect(names).toContain("request:/render");
    expect(names).toContain("render:pnl-chart");
  });

  it("declares a fragment budget", () => {
    expect(pnlChartBudget).toMatchObject({
      scope: "fragment",
