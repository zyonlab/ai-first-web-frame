...
import { renderPnlChart } from "./render";
...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderPnlChart({
      ctx: { locale: "en-US" },
      props: { symbol: "BTC", range: "24h" },
    });
    reply.type("text/html; charset=utf-8");
    return renderServiceHome({
      title: "pnl-chart fragment",
      port: DEFAULT_PORT,
  ...
  server.get("/manifest", async () => pnlChartManifest);
  server.get("/assets", async () => pnlChartManifest.assets);
  server.get("/budget", async () => pnlChartBudget);
  server.post("/render", async (request, reply) => {
    ...
    const body = parsed.request as Parameters<typeof renderPnlChart>[0];
    ...
    const result = await renderPnlChart(body, { trace });
...
if (
  ...
  console.log(`pnl-chart listening on ${port}`);
}
