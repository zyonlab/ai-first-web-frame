...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderOpenOrders({
      ctx: { locale: "en-US" },
      props: { symbol: "BTC" },
    });
    reply.type("text/html; charset=utf-8");
    return renderServiceHome({
      title: "open-orders fragment",
      port: DEFAULT_PORT,
  ...
  server.get("/manifest", async () => openOrdersManifest);
  server.get("/assets", async () => openOrdersManifest.assets);
  server.get("/budget", async () => openOrdersBudget);
  server.post("/render", async (request, reply) => {
    ...
    const body = parsed.request as Parameters<typeof renderOpenOrders>[0];
    ...
    const result = await renderOpenOrders(body, { trace });
...
if (
  ...
  console.log(`open-orders listening on ${port}`);
}
