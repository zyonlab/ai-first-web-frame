...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderPositionsTable({ ctx: { locale: "en-US" } });
    reply.type("text/html; charset=utf-8");
    return renderServiceHome({
      title: "positions-table fragment",
      port: DEFAULT_PORT,
  ...
  server.get("/manifest", async () => positionsTableManifest);
  server.get("/assets", async () => positionsTableManifest.assets);
  server.get("/budget", async () => positionsTableBudget);
  server.post("/render", async (request, reply) => {
    ...
    const body = parsed.request as Parameters<typeof renderPositionsTable>[0];
    ...
    const result = await renderPositionsTable(body, { trace });
...
if (
  ...
  console.log(`positions-table listening on ${port}`);
}
