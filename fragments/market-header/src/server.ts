...
export type BuildServerOptions = {
  /** Injectable clock for deterministic uptime + countdown in tests. */
  now?: () => number;
...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderMarketHeader({
      ctx: { locale: "en-US" },
      props: { symbol: "BTC" },
    });
    reply.type("text/html; charset=utf-8");
    return renderServiceHome({
      title: "market-header fragment",
      port: 4203,
  ...
  server.get("/manifest", async () => marketHeaderManifest);
  server.get("/assets", async () => marketHeaderManifest.assets);
  server.get("/budget", async () => marketHeaderBudget);
  server.post("/render", async (request, reply) => {
    ...
    const body = parsed.request as Parameters<typeof renderMarketHeader>[0];
    ...
    const result = await renderMarketHeader(body, { trace, now });
...
if (
  ...
  console.log(`market-header listening on ${port}`);
}
