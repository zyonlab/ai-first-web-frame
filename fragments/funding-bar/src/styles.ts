/**
 * Scoped CSS for the funding-bar fragment — single source of truth.
 *
 ...
 * composed page never fetches the standalone /assets/funding-bar.css file.
 *
 * Keep this in sync with assets/funding-bar.css (or src/funding-bar.css); the .css file
 * is retained so the css-budget audit still measures the fragment stylesheet.
 */
export const fundingBarCss = `/**
...
[data-fragment="funding-bar"] .funding-bar__item {
  display: flex;
  flex-direction: column;
  gap: 2px;
}

[data-fragment="funding-bar"] .funding-bar__label {
...
[data-fragment="funding-bar"] .funding-bar__value {
...
[data-fragment="funding-bar"] .funding-bar__countdown > time {
  position: relative;
}

[data-fragment="funding-bar"] .funding-bar__countdown > time::after {
  content: "";
  position: absolute;
  top: 50%;
  right: -10px;
...
[data-fragment="funding-bar"][data-fallback="true"] {
  color: var(--mvp-color-muted, rgba(128, 128, 128, 0.9));
}

@keyframes funding-bar-pulse {
  0%,
  100% {
    opacity: 0.35;
  }
