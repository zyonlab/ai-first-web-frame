...
import { renderFundingBar } from "./render";
...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderFundingBar({
      ctx: { locale: "en-US" },
      props: { symbol: "BTC" },
    });
    reply.type("text/html; charset=utf-8");
    return renderServiceHome({
      title: "funding-bar fragment",
      port: DEFAULT_PORT,
  ...
  server.get("/manifest", async () => fundingBarManifest);
  ...
  server.post("/render", async (request, reply) => {
    ...
    const result = await renderFundingBar(body, { trace });
...
if (
  ...
  console.log(`funding-bar listening on ${port}`);
}
