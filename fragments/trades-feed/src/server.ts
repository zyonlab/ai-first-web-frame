...
import { renderTradesFeed } from "./render";
...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderTradesFeed({
      ctx: { locale: "en-US" },
      props: { symbol: "BTC", limit: 10 },
    });
    reply.type("text/html; charset=utf-8");
    return renderServiceHome({
      title: "trades-feed fragment",
      port: DEFAULT_PORT,
  ...
  server.get("/manifest", async () => tradesFeedManifest);
  server.get("/assets", async () => tradesFeedManifest.assets);
  server.get("/budget", async () => tradesFeedBudget);
  server.post("/render", async (request, reply) => {
    ...
    const body = parsed.request as Parameters<typeof renderTradesFeed>[0];
    ...
    const result = await renderTradesFeed(body, { trace });
...
if (
  ...
  console.log(`trades-feed listening on ${port}`);
}
