...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderPromotionBanner({
      ctx: { locale: "en-US" },
  ...
  server.post("/render", async (request, reply) => {
    ...
    const body = parsed.request as Parameters<typeof renderPromotionBanner>[0];
    ...
    const result = await renderPromotionBanner(body, { trace });
