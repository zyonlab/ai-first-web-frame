...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderRecommendationWidget({
      ctx: { locale: "en-US" },
  ...
  server.post("/render", async (request, reply) => {
    ...
    const body = parsed.request as Parameters<
      typeof renderRecommendationWidget
    >[0];
    ...
    const result = await renderRecommendationWidget(body, { trace });
