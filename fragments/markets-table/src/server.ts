...
import { renderMarketsTable } from "./render";
...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderMarketsTable({ ctx: { locale: "en-US" } });
    reply.type("text/html; charset=utf-8");
    return renderServiceHome({
      title: "markets-table fragment",
      port: DEFAULT_PORT,
  ...
  server.get("/manifest", async () => marketsTableManifest);
  server.get("/assets", async () => marketsTableManifest.assets);
  server.get("/budget", async () => marketsTableBudget);
  server.post("/render", async (request, reply) => {
    ...
    const body = parsed.request as Parameters<typeof renderMarketsTable>[0];
    ...
    const result = await renderMarketsTable(body, { trace });
...
if (
  ...
  console.log(`markets-table listening on ${port}`);
}
