...
import { renderAccountBar } from "./render";

const DEFAULT_PORT = 4207;
...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderAccountBar({
      ctx: { locale: "en-US" },
      props: {},
    });
    reply.type("text/html; charset=utf-8");
    return renderServiceHome({
      title: "account-bar fragment",
      port: DEFAULT_PORT,
  ...
  server.get("/manifest", async () => accountBarManifest);
  server.get("/assets", async () => accountBarManifest.assets);
  server.get("/budget", async () => accountBarBudget);
  server.post("/render", async (request, reply) => {
    ...
    const body = parsed.request as Parameters<typeof renderAccountBar>[0];
    ...
    const result = await renderAccountBar(body, { trace });
...
if (
  ...
  console.log(`account-bar listening on ${port}`);
}
