...
export function buildServer(options: BuildServerOptions = {}) {
  ...
  server.get("/", async (_request, reply) => {
    const demo = await renderChartPanel({
      ctx: { locale: "en-US" },
      props: { symbol: "BTC", interval: "1m" },
    });
    reply.type("text/html; charset=utf-8");
    return renderServiceHome({
      title: "chart-panel fragment",
      port: DEFAULT_PORT,
  ...
  server.get("/manifest", async () => chartPanelManifest);
  server.get("/assets", async () => chartPanelManifest.assets);
  server.get("/budget", async () => chartPanelBudget);
  server.post("/render", async (request, reply) => {
    ...
    const body = parsed.request as Parameters<typeof renderChartPanel>[0];
    ...
    const result = await renderChartPanel(body, { trace });
...
if (
  ...
  console.log(`chart-panel listening on ${port}`);
}
