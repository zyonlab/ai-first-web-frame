...
import { createTradeStore, useStoreSlice } from "./store";

/**
 * Generic slice map used only by the tests. The store is generic over the
 * slice shape; concrete trade slices (activeSymbol, orderDraft, ...) are
 * defined by the P1 data agent, not here.
 */
type TestSlices = {
  activeSymbol: string;
  count: number;
...
const contracts: InteractionContract[] = [
  {
    ...
    publisher: "trade-store",
    subscribers: ["trade-store"],
    payloadSchema: {},
  },
  {
    channel: "count",
...
describe("createTradeStore", () => {
  it("returns the initial value for a slice", () => {
    const store = createTradeStore<TestSlices>(contracts, {
  ...
  it("throws when a slice has no declared interaction contract", () => {
    const store = createTradeStore<TestSlices>([contracts[0]], {
      initial: { activeSymbol: "BTC", count: 0 },
    });
...
describe("SSR safety", () => {
  it("createTradeStore works without a window (degraded pub/sub only)", async () => {
